...
function NavBar() {
  ...
  const [filteredMeals, setFilteredMeals] = useState(allReceipes? allReceipes : []);
  
  useEffect(()=>{
     if(submtinput.length>0){
    axios.get(`https://www.themealdb.com/api/json/v1/1/search.php?s=${submtinput}`)
    .then((data)=> {
...
useEffect(()=>{
  ...
  const meals = product.map((item) => Array.from({length: 20}, (_, id) => ([value])
  .includes(item[`strIngredient${id+1}`]) ? item :'')).flat().filter((item) => item !== '')
...
<>
    <div className='nav-parent' >
       <div className='search-container'>
        <input type='text' value={searchinput} placeholder='What do you want to cook receipe?' onChange={(e)=>handleChange(e)}/>
        <button className='search-button' onClick={()=>{handleSumbit()}}>Search</button>
        </div>
        <div className='select-div'>
        <h2>Fliter by</h2>
        <div className='select-parent'>
        <select onChange={(e)=>{handleIngredient(e)}}>
             <option value="ingredients" defaultValue={"ingredients"}>Ingredients</option>
             {
               ingredient.map((item)=>
               <option value={item.strIngredient} key={item.idIngredient}>{item.strIngredient}</option>
               )
             }
        </select>
        ...
        <select onChange={(e)=>{handleMealType(e)}}>
             <option value="meal types" defaultValue={"meal types"}>Meal Types</option>
              {
               mealtype.map((item)=>
 ...
 {filteredMeals.length > 0 && <Home product={filteredMeals} />} 
 </>
  )
}
